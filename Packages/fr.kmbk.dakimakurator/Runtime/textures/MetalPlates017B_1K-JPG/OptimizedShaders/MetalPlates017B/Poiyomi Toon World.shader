Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/538e376707e71ab47be1a6e2e3225d10"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(2.478431,1.898039,4,1).rgb, 0.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(2.478431,1.898039,4,1).rgb, 0.0);
				#endif
				...
				if (1.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(0.6, 1.0, 4.0, 0.0);
				}
				if(0.0)
				{
