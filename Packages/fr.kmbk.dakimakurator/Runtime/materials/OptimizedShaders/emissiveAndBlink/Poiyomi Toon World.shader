Shader "Hidden/Locked/.poiyomi/Poiyomi Toon World/fa227753a404bcf4287ec69203c84364"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			float3 applyEmission(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiLight poiLight, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				emissionColor0 = emissionTex.rgb * lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(1.856394,0.4934716,2.996079,1).rgb, 0.0);
				emissionAlpha = emissionTex.a;
				#else
				emissionColor0 = lerp(1, poiFragData.baseColor, mixBaseColor).rgb * poiThemeColor(poiMods, float4(1.856394,0.4934716,2.996079,1).rgb, 0.0);
				#endif
				...
				if (1.0)
				{
					emissionStrength0 *= calculateBlinkingEmission(0.8, 1.0, 4.0, 0.0);
				}
				if(0.0)
				{
